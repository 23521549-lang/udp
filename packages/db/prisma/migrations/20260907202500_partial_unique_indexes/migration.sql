-- ============================================================
-- Partial unique index — Prisma không diễn đạt được mệnh đề WHERE, nên phải
-- viết tay. Bốn index này cưỡng chế bất biến ở TẦNG DATABASE, không phụ thuộc
-- vào việc code có nhớ kiểm tra hay không.
--
-- Vì sao điều đó quan trọng: mỗi bất biến dưới đây, nếu vỡ, đều gây hậu quả
-- TỐN TIỀN THẬT hoặc MẤT QUYỀN, và cả bốn đều là race condition — tức là loại
-- lỗi mà kiểm tra bằng SELECT-rồi-INSERT trong code không chặn được.
-- ============================================================
...
-- Chặn kịch bản bấm nút Provision hai lần tạo ra hai cluster trên tài khoản
-- BYOC của developer (ADR-02). Đây là bất biến đắt nhất nếu vỡ.
...
-- COALESCE vì mỗi loại rollout để NULL ở cột của loại kia; trong unique index,
-- NULL được coi là khác nhau nên không chặn được nếu không ép về chuỗi rỗng.
-- Hai reconciler cùng điều khiển một target là đúng thứ ADR-01 sinh ra để tránh.
