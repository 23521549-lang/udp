...
import { modelToTableMap, openClient, readDbColumns, type DbColumn } from "../src/db-schema.js";
...
function dbColumnsOf(entity: string): string[] {
  ...
  if (table === undefined) throw new Error(`Không có model ${entity} trong schema.prisma`);
  const cols = dbColumns.get(table);
  if (cols === undefined) throw new Error(`Không có bảng ${table} trong database`);
...
describe("A2 — §2.1 ERD khớp database", () => {
  it("tập entity của ERD bằng tập model", () => {
    expect([...erd.map((e) => e.name)].sort()).toEqual([...modelToTable.keys()].sort());
  });
  ...
  it("mỗi entity có đủ cột, không thừa không thiếu", () => {
    ...
    for (const entity of erd) {
      ...
      if (missing.length) problems.push(`${entity.name} THIẾU: ${missing.join(", ")}`);
      if (extra.length) problems.push(`${entity.name} THỪA: ${extra.join(", ")}`);
    }
...
describe("A3 — §2.2 bảng cột khớp database", () => {
  it("mỗi heading §2.2 ứng với một model", () => {
    const unknown = docTables.map((t) => t.name).filter((n) => !modelToTable.has(n));
    expect(unknown).toEqual([]);
  });

  it("mọi model đều được §2.2 mô tả", () => {
    const described = new Set(docTables.map((t) => t.name));
    const undocumented = [...modelToTable.keys()].filter((m) => !described.has(m)).sort();
  ...
  it("mỗi bảng có đủ cột, không thừa không thiếu", () => {
    ...
    for (const table of docTables) {
      ...
      const missing = [...actual].filter((c) => !declared.has(c)).sort();
      const extra = [...declared].filter((c) => !actual.has(c)).sort();
      if (missing.length) problems.push(`${table.name} THIẾU: ${missing.join(", ")}`);
      if (extra.length) problems.push(`${table.name} THỪA: ${extra.join(", ")}`);
    }
    expect(problems).toEqual([]);
  });
  ...
  it("cột khai NOT NULL trong tài liệu thì database cũng NOT NULL", () => {
    ...
    for (const table of docTables) {
      ...
      const byName = new Map((dbColumns.get(dbName) ?? []).map((c) => [c.name, c]));

      for (const col of table.columns) {
        const actual = byName.get(col.name);
        if (actual === undefined) continue;
        const docSaysNotNull = /NOT NULL/i.test(col.constraint) && !/NULLABLE/i.test(col.constraint);
        if (docSaysNotNull && actual.nullable) {
          problems.push(`${table.name}.${col.name}: tài liệu nói NOT NULL, database cho NULL`);
        }
      }
    }
    expect(problems).toEqual([]);
...
describe("A2/A3 — hai mục của tài liệu phải nói cùng một điều", () => {
  it("ERD và §2.2 mô tả cùng tập cột cho mỗi bảng", () => {
    ...
    for (const entity of erd) {
      ...
      if (onlyTable.length) problems.push(`${entity.name}: §2.2 có nhưng ERD không — ${onlyTable.join(", ")}`);
      if (onlyErd.length) problems.push(`${entity.name}: ERD có nhưng §2.2 không — ${onlyErd.join(", ")}`);
    }
    expect(problems).toEqual([]);
